...
- The working database contains some rare erroneous characters if generated from Matlab (268 exactly into the whole database in 2024). From GNU Octave everything is OK. I search a way of fixing this. The reason is a difference between Matlab and GNU Octave when dealing with UTF-8 encoding. It anyway does not affect the code performance for its only task: finding references.
